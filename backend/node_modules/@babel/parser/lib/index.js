...
var flow = superClass => class FlowParserMixin extends superClass {
  ...
  jsxParseOpeningElementAfterName(node) {
    ...
    return super.jsParseOpeningElementAfterName(node);
  }
  isLookaheadToken_lt() {
...
var jsx = superClass => class JSXParserMixin extends superClass {
  ...
  jsxParseNamespacedName() {
    const startLoc = this.state.startLoc;
    const name = this.jsParseIdentifier();
    if (!this.eat(14)) return name;
    const node = this.startNodeAt(startLoc);
    node.namespace = name;
    node.name = this.jsParseIdentifier();
    return this.finishNode(node, "JSXNamespacedName");
  }
  jsxParseElementName() {
    const startLoc = this.state.startLoc;
    let node = this.jsParseNamespacedName();
    ...
    while (this.eat(16)) {
      ...
      newNode.object = node;
      newNode.property = this.jsParseIdentifier();
  ...
  jsxParseAttributeValue() {
    ...
    switch (this.state.type) {
      case 5:
        ...
        this.next();
        node = this.jsParseExpressionContainer(node, types.j_oTag);
  ...
  jsxParseExpressionContainer(node, previousContext) {
    if (this.match(8)) {
      node.expression = this.jsParseEmptyExpression();
    } else {
  ...
  jsxParseAttribute() {
    ...
    node.name = this.jsParseNamespacedName();
    node.value = this.eat(29) ? this.jsParseAttributeValue() : null;
  ...
  jsxParseOpeningElementAt(startLoc) {
    ...
    node.name = this.jsParseElementName();
    return this.jsParseOpeningElementAfterName(node);
  }
  jsxParseOpeningElementAfterName(node) {
    const attributes = [];
    while (!this.match(56) && !this.match(144)) {
      attributes.push(this.jsParseAttribute());
    }
  ...
  jsxParseElementAt(startLoc) {
    const node = this.startNodeAt(startLoc);
    const children = [];
    const openingElement = this.jsParseOpeningElementAt(startLoc);
      ...
      contents: for (;;) {
        switch (this.state.type) {
          case 143:
            ...
            this.next();
            if (this.eat(56)) {
              closingElement = this.jsParseClosingElementAt(startLoc);
              break contents;
            }
            children.push(this.jsParseElementAt(startLoc));
            break;
          case 142:
          ...
          case 5:
            {
              ...
              this.next();
              if (this.match(21)) {
                children.push(this.jsParseSpreadChild(node));
              } else {
                children.push(this.jsParseExpressionContainer(node, types.j_expr));
              }
              break;
            }
          default:
  ...
  jsxParseElement() {
    const startLoc = this.state.startLoc;
    this.next();
    return this.jsParseElementAt(startLoc);
  }
